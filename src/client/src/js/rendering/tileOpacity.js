...
export default {
	...
	map (tile) {
		...
		let alpha = opacityConfig.default;
		const tileConfig = opacityConfig[mappedTile];
		if (tileConfig)
			alpha = tileConfig;
		else if (opacityConfig.min !== undefined)
			alpha = opacityConfig.min + (Math.random() * opacityConfig.maxAdd);
