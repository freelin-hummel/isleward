...
import tplTab from './tplTab.html?raw';
import './styles.css';
import messagesMobile from './mobile';
...
export default {
	...
	onGetCustomChatChannels (channels) {
		...
		messages.forEach(m => {
			...
			if (m.type) {
				...
				if (isMobile && ['loot', 'info'].indexOf(m.type) !== -1)
					events.emit('onGetAnnouncement', { msg: m.message });
			}
		});
