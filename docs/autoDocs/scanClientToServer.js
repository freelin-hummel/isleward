...
function extractClientRequest (content, file) {
	const results = [];
	let i = 0;
	while (i < content.length) {
		const callStart = content.indexOf('client.request(', i);
		...
		if (end !== -1) {
			...
			const argsStr = call.substring('client.request('.length, call.length - 1);
...
for (const file of files) {
	if (file.includes('scanClientToServer.js')) continue;
	console.log(`Processing ${file}...`);
	const content = fs.readFileSync(file, 'utf8');
	const results = extractClientRequest(content, file);
...
let output = '# Events: Client -> Server\n\n';
for (const eventName in events) {
	const e = events[eventName];
	output += '## Event: client.request\n';
	output += `${dedent(eventName)}\n\n`;
	output += `* Files\n${e.files.map(f => '  - ' + f).join('\n')}\n`;
	output += '_____________________________________\n\n';
}

fs.writeFileSync('docs/autoDocs/output/clientToServer.txt', output);

console.log('Done. Results in docs/autoDocs/output/clientToServer.txt');
